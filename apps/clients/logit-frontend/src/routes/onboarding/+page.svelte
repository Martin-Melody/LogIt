<script lang="ts">
  ...
  const PRESETS: Preset[] = [
    {
      ...
      days: [
        {
          name: "Push",
          exercises: ["Bench Press", "Overhead Press", "Lateral Raise", "Tricep Pushdown"],
        },
        {
          name: "Pull",
          exercises: ["Pull-Up", "Barbell Row", "Bicep Curl", "Rear Delt Fly"],
        },
        {
          name: "Legs",
    ...
    },
    {
      ...
      days: [
        {
          name: "Upper",
          exercises: ["Bench Press", "Barbell Row", "Overhead Press", "Bicep Curl", "Tricep Pushdown"],
        },
        {
          name: "Lower",
          exercises: ["Squat", "Romanian Deadlift", "Leg Press", "Leg Curl", "Calf Raise"],
    ...
    },
    {
      id: "blank",
      name: "Start blank",
      description: "Create your own split from scratch",
      days: [],
    },
  ];
  ...
  async function finishOnboarding() {
    // Reset tours so the home tour runs automatically on first visit
    resetTours();
    onboarding.complete();
    await goto("/");
  }
  ...
  async function onEnableNotifications() {
    try {
      ...
      // Web or not supported — fine to continue
    }
      ...
      <ul class="flex flex-col gap-2 flex-1">
        {#each PRESETS as preset (preset.id)}
          <li>
            <button
              type="button"
              class="w-full text-left px-4 py-3 rounded border transition-colors {selectedPreset === preset.id
