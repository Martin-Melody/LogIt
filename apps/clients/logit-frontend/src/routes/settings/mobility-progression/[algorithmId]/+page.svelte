<script lang="ts">
  ...
  import { getMobilityAlgorithmRegistry, getProgressionRepo } from "$lib/data/repoProvider";
  import type {
  ...
  let algorithm = $state<MobilityProgressionAlgorithm | null>(null);
  ...
  onMount(async () => {
    try {
      const [algo, stored] = await Promise.all([
        getMobilityAlgorithmRegistry().get(algorithmId),
  ...
  {:else if algorithm?.preferencesSchema?.length}
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description>
          These settings are passed to the algorithm when it generates mobility suggestions.
        </Card.Description>
      </Card.Header>
      <Card.Content>
