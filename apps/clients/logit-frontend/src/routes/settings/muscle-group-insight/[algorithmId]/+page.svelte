<script lang="ts">
  ...
  import { getMuscleGroupInsightAlgorithmRegistry, getProgressionRepo } from "$lib/data/repoProvider";
  import type {
  ...
  let algorithm = $state<MuscleGroupInsightAlgorithm | null>(null);
  ...
  onMount(async () => {
    try {
      const [algo, stored] = await Promise.all([
        getMuscleGroupInsightAlgorithmRegistry().get(algorithmId),
...
<div class="flex flex-col gap-3 p-3 pb-24">
  <div class="flex items-center gap-2">
    ...
    <h1 class="text-base font-semibold">{algorithm?.name ?? "Algorithm"} Settings</h1>
  </div>
  ...
  {:else if algorithm?.preferencesSchema?.length}
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description>
          These settings are passed to the algorithm when it analyzes your muscle-group volume.
        </Card.Description>
      </Card.Header>
      <Card.Content>
