      ...
      {#if field.type === "select" && field.options}
        ...
        <div class="flex rounded border overflow-hidden text-xs w-full">
          {#each field.options as opt (opt.value)}
            <button
              type="button"
              class="flex-1 px-3 py-1.5 text-center transition-colors {getStr(field.key) === String(opt.value)
        ...
        <div class="flex items-center justify-between gap-3">
          ...
          {#if field.type === "boolean"}
            <button
              ...
              class="relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors {getBool(
                field.key,
              )
                ? 'bg-primary'
                : 'bg-input'}"
            ...
            >
              <span
                class="pointer-events-none block h-4 w-4 rounded-full bg-background shadow-lg transition-transform {getBool(
                  field.key,
                )
                  ? 'translate-x-4'
                  : 'translate-x-0'}"
