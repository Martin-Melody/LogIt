<script lang="ts">
  ...
  import { kgToDisplay, type WeightUnit } from "./nutrition";
  ...
  const model = $derived.by(() => {
    ...
    return { line, dots, targetY, min, max };
  });
</script>

{#if model}
  <svg viewBox="0 0 {W} {H}" class="w-full" role="img" aria-label="Bodyweight trend">
    ...
    {#each model.dots as d}
      <circle cx={d.cx} cy={d.cy} r="1.6" class="fill-muted-foreground/50" />
    {/each}
    <path d={model.line} fill="none" class="stroke-primary" stroke-width="1.75" stroke-linejoin="round" />
  </svg>
{:else}
